            ...
            <div class="panel panel-inverse" data-sortable-id="form-validation-1">
                ...
                <div class="panel-body panel-form">
                    <form class="form-horizontal form-bordered" action="{{ url($link.'/'.$item['id']) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group"><!-- 语言 -->
                            <label class="control-label col-md-3 col-sm-3" for="language">语言 :</label>
                            <div class="col-md-8 col-sm-8">
                                <select class="form-control" name="language" id="language">
                                    @if ( isset($languages) && $languages )
                                        @foreach ( $languages as $value )
                                            <option value="{{ $value }}" @if ( ($item['language']??'english')==$value ) selected="selected" @endif>{{ $value }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
