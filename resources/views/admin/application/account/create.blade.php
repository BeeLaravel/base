                ...
                <div class="panel-body panel-form">
                    <form class="form-horizontal form-bordered" action="{{ url($link) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group"><!-- 链接 -->
                            ...
                            <div class="col-md-8 col-sm-8">
                                <input class="form-control" type="text" name="url" value="{{ old('url') }}" placeholder="链接" data-parsley-required="true" id="url" />
                            </div>
                        </div>
                        <div class="form-group"><!-- 账号 -->
                            <label class="control-label col-md-3 col-sm-3" for="account">账号 * :</label>
                            <div class="col-md-8 col-sm-8">
                                <input class="form-control" type="text" name="account" value="{{ old('account') }}" placeholder="账号" data-parsley-required="true" id="account" />
                            </div>
                        </div>
                        <div class="form-group"><!-- 密码 -->
                            <label class="control-label col-md-3 col-sm-3" for="password">密码 * :</label>
                            <div class="col-md-8 col-sm-8">
                                <input class="form-control" type="text" name="password" value="{{ old('password') }}" placeholder="密码" data-parsley-required="true" id="password" />
                            </div>
                        </div>
                                ...
                                <select class="form-control" name="type" id="type">
                                    @if ( $types )
                                        @foreach ( $types as $value )
                                            <option value="{{ $value }}" @if ( (old('type')??'Other')==$value ) selected="selected" @endif>{{ $value }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
